<x-admin-layout>
        ...
        <div class="page-header mb-0">
            <div class="row">
                ...
                <div class="col-md-6 col-sm-12 text-right">
                    ...
                    <a href="{{ route('admin.players.create') }}" class="btn btn-primary btn-sm scroll-click"><i class="fa fa-plus"></i> {{ __('admin/player.index.create') }}</a>
                    @endif
                    @if(AdminCan('user-forcedelete'))
                    <a href="{{ route('admin.players.archive') }}" class="btn btn-primary btn-sm scroll-click">{{ __('admin/player.index.archive') }}</a>
                    @endif
                </div>
            </div>
        </div>
    ...
    <div class="py-12">
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
            <div class="p-6 text-gray-900">
                <div class="row">
                    <div class="col-md-12 col-sm-12">
                        <div class="clearfix mb-10">
                            <div class="pull-left">
                                <h4 class="text-blue h4">{{ __('admin/player.index.title') }}</h4>
                            </div>
                        </div>
                ...
                <table class="table table nowrap table-bordered table-striped no-footer dtr-inline" id="players-table">
                    <thead>
                    <tr>
                        <th scope="col">#</th>
                        <th scope="col">{{ __('admin/common.table.player_name') }}</th>
                        <th scope="col">{{ __('admin/common.table.player_num') }}</th>
                        <th scope="col">{{ __('admin/common.table.player_wieght') }}</th>
                        <th scope="col">{{ __('admin/common.table.player_team') }}</th>
                        <th scope="col">{{ __('admin/common.table.player_category') }}</th>
    ...
    <script type="text/javascript">
        ...
        $('#players-table tbody').on('click', '#deleteBtn', function(argument) {
    ...
    @section('page_title'){{ __('admin/player.index.title_tag') }}@endsection
</x-admin-layout>
