<x-admin-layout>
                ...
                <div class="col-md-6 col-sm-12">
                    <nav aria-label="breadcrumb" role="navigation">
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item">
                                <a href="{{ route('admin.players.index') }}">{{ __('admin/player.index.title') }}</a>
                            </li>
                            <li class="breadcrumb-item active" aria-current="page">
                                {{ __('admin/player.archive.title') }}
                            </li>
                        </ol>
                    </nav>
                </div>
                <div class="col-md-6 col-sm-12 text-right">
                    <a href="{{ route('admin.players.index') }}" class="btn btn-primary btn-sm scroll-click">
                        {{ __('admin/player.archive.all') }}</a>
                </div>
            </div>
        </div>
    </x-slot>
                ...
                <div class="row">
                    <div class="col-md-12 col-sm-12">
                        <div class="clearfix mb-10">
                            <div class="pull-left">
                                <h4 class="text-blue h4">{{ __('admin/player.archive.title') }}</h4>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    <script type="text/javascript">
        $(function() {
            var table = $('#players-table').DataTable({
                ...
                processing: true,
                serverSide: true,
                ajax: "{{ Route('admin.players.trashed') }}",
                columns: [{
                    data: 'id',
                    name: 'id',
                    orderable: false,
    ...
    @endpush
    @section('page_title'){{ __('admin/player.archive.title_tag') }}@endsection
</x-admin-layout>
